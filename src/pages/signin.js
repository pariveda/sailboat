...
const Signin = React.createClass({
    _signin: function(e) {
        ...
        $.ajax({
            type: 'POST',
            url: 'https://j0rx4mlpt8.execute-api.us-west-2.amazonaws.com/dev/authenticate',
            contentType: 'application/json',
            data: JSON.stringify({email:email, password:password}),
            success: function(result) {
                console.log(result);
                sessionStorage.setItem("isSignedIn", true);
                // sessionStorage.setItem("baseURL", result.profile.clouds[0].baseURL);
                window.location = "/#";
            }.bind(this),
            ...
            rel('div', {id:'wrapper'},[
                rel('nav', {id:'nav-wrapper', className:'navbar navbar-default navbar-fixed-top', role:'navigation'}, [
                    rel('div', {id:'top-menu-wrapper'}, [
                        rel('div', {className:'navbar-header'}, rel('span', {}, [
                            rel('a', {className:'navbar-brand', href:'/#signin'}, 'GrokeryLab')
                        ]))
                    ])
                ]),
                rel('div', {id:'page-wrapper'}, [
                    rel('form', {className:'form-signin'}, [
                        ...
                        rel('button', {className:'btn btn-lg btn-primary btn-block', type:'submit', onClick:this._signin}, 'Sign in')
                    ])
                ])
            ])
        );
